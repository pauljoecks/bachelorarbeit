...
Namespace VB
    ...
    Public Class VBscanCNONTROLSample
        ...
        Shared Sub GetProfiles_Callback()
            ...
            Dim m_vucModuleResult(uiResolution) As UInteger
            Dim m_vucModuleResultBuffer(uiResolution * 4) As Byte
            Dim offset As Double = 85.0
            ...
            iRetValue = CLLTI.ConvertProfile2ModuleResult(hLLT, abyProfileBuffer, abyProfileBuffer.Length, m_vucModuleResultBuffer, m_vucModuleResultBuffer.Length, tpartialProfile)
            If (iRetValue < CLLTI.GENERAL_FUNCTION_OK) Then
                OnError("Error during ConvertProfile2ModuleResult", iRetValue)
                Return
            End If

            'Convert m_vucModuleResultBuffer to an unsigned int Buffer
            'Data format: little endian
            For i As Integer = 0 To uiResolution - 1 Step 1
                m_vucModuleResult(i) = (CUInt(m_vucModuleResultBuffer(i * 4 + 3)) << 24) + (CUInt(m_vucModuleResultBuffer(i * 4 + 2)) << 16) + (CUInt(m_vucModuleResultBuffer(i * 4 + 1)) << 8) + (CUInt(m_vucModuleResultBuffer(i * 4)))
            Next
        ...
        Shared Function GetAppendedResults(ByRef vucModuleResult() As UInteger, nModuleResultSize As Integer, littleEndian As Boolean, offset As Double, scaling As Double) As Integer
            ...
            Next
            Dim validHeaders As UInteger() = New UInteger() {2, 4, 6, 7, 11, 13, 14, 15, 128, 129, 130}
            ...
            For i = 0 To nModuleResultSize - 1
                ...
                If (Not littleEndian) Then
                    ...
                    ElseIf (validHeaders.Contains((vucModuleResult(i) >> 24) And &HFF)) Then ' header Then valid
                        ' Add length of result block to i to skip results of current module
                        i += CInt(vucModuleResult(i) And &HFF) - 1

                    ElseIf (vucModuleResult(i) = 0) Then ' Queue End
                    ...
                    Select Case (type)
                        ...
                        Case 5
                            ...
                            If (length_16_32) Then
                                '32 bit length
                                Console.Write("(32bit): ")
                                Dim tmp As UInt32 = mv_data
                                distance = Convert.ToInt32(tmp.ToString("X8"), 16) * scaling
                            Else
                                If (upper_lower) Then
                                    'distance in lower 16 bit
                                    Console.Write("(16bit lower): ")
                                    Dim tmp As UInt32 = mv_data And &HFFFF
                                    distance = Convert.ToInt16(tmp.ToString("X4"), 16) * scaling
                                Else
                                    ' distance in upper 16 bit
                                    Console.Write("(16bit upper): ")
                                    Dim tmp As UInt32 = (mv_data And &HFFFF0000) >> 16
                                    distance = Convert.ToInt16(tmp.ToString("X4"), 16) * scaling
                                End If
                        ...
                        Case 6
                            ...
                            If (length_16_32) Then
                                ...
                                Console.Write("(32bit): ")
                                Dim tmp As UInt32 = mv_data
                                angle = Convert.ToInt32(tmp.ToString("X8"), 16) * 0.01
                            Else
                                If (upper_lower) Then
                                    'angle in lower 16 bit
                                    Console.Write("(16bit lower): ")
                                    Dim tmp As UInt32 = mv_data And &HFFFF
                                    angle = Convert.ToInt16(tmp.ToString("X4"), 16) * 0.01
                                Else
                        ...
                        Case 8
                            ...
                            If (length_16_32) Then
                                ...
                                x = Convert.ToInt32((vucModuleResult(ptr_x)) - 32768) * scaling
                                Dim ptr_z As UInteger = (mv_mask And &HFFF0) >> 8
                                z = Convert.ToInt32((vucModuleResult(ptr_z)) - 32768) * scaling + offset
                                Console.Write(x & " , " & z)
